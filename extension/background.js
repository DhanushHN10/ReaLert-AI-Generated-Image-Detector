...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === "check-ai-image") {
    ...
    chrome.storage.local.get(["selectedModel"], (result) => {
      ...
      else if (result.selectedModel === "model3") {
        checkImage(imageUrl);
      }
    });
  }
...
async function checkImageOwnModel2(imageUrl) {
  ...
  chrome.storage.local.set({ checking: false });
  console.log("Result from own model:", data);
  var category = data.prediction.category || "unknown";
  const probs = data.prediction.probabilities;
  const highest = Object.entries(probs).reduce((a, b) => (b[1] > a[1] ? b : a));
  category = highest[0];
  if (category === "Real") {
    const prob = data.prediction.confidence || 0;
    console.log("REAL");
...
async function checkImage(imageUrl) {
  ...
  try {
    ...
    var prob = data?.type?.ai_generated * 100 || 0;

    const category = prob > 50 ? "AI-generated" : "Real";
    if (prob < 50) {
      prob = 100 - prob;
    }
    chrome.runtime.sendMessage({
