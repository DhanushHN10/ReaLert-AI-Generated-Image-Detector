...
chrome.runtime.onInstalled.addListener(() => {
    chrome.contextMenus.create({
        id: "check-ai-image",
        title: "Check if image is AI-generateddd",
        contexts: ["image"]
    });
});
...
async function checkImageOwnModel(imageUrl) {
    ...
    if (category === "Real") {
        ...
        console.log("REAL");
        chrome.runtime.sendMessage({ prob: (100 - prob), imageUrl: imageUrl, category: category });
        chrome.storage.local.set({ lastProb: prob, lastImage: imageUrl, category: category });
        ...
        chrome.runtime.sendMessage({ prob: (prob), imageUrl: imageUrl, category: category });
        chrome.storage.local.set({ lastProb: prob, lastImage: imageUrl, category: category });
        chrome.storage.local.get({ history: [] }, (result) => {
            const history = result.history;
            history.unshift({ url: imageUrl, prob, category, timestamp: Date.now() }); // latest first
...
async function checkImage(imageUrl) {
    ...
    try {
        ...
        chrome.notifications.create({
            ...
            iconUrl: "icon.png",
            title: "AI Image Detection",
            message: (data.status == "success") ? `AI-generated likelihood: ${(prob * 100).toFixed(2)}%` : "Failed"
        });
...
const HF_API_URL = "https://router.huggingface.co/hf-inference/models/openai-community/roberta-base-openai-detector";
